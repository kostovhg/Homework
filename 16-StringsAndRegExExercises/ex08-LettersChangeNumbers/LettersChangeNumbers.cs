...
namespace ex08_LettersChangeNumbers
{
    class LettersChangeNumbers
    {
        static void Main(string[] args)
        {
            ...
            {
                if (inReg.IsMatch(input)) inputs.Add(input);
            }
        ...
        private static double CalculateNumbersWithLetters(string str)
        {
            ...
            if (firstL <= 'Z') res = num / (firstL - 'A' + 1);
            else res = num * (firstL - 'a' + 1);

            if (lastL <= 'Z') res -= lastL - 'A' + 1;
            else res += lastL - 'a' + 1;
